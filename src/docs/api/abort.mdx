...
This BotAction is similar to a [pipe()()](/api/assembly-lines#pipe), except it will restart when an assembled BotAction returns an `AbortLineSignal` with `assembledLines = 1` (default). In this event, it takes the pipe value from the `AbortLineSignal` then injects it into the first assembled BotAction, as it restarts the pipe.

> You can use Restart to rerun assembled BotActions with a different initial pipe value by setting it in the `AbortLineSignal` with `assembledLines` equal to one.
...
Here is an example with a made-up BotAction called `goToPipeValue()` that would use the pipe value as the URL, to navigate too before running subsequent chained BotActions:
```typescript
...
```
In this case, all the BotActions after `goToPipeValue` could do a common routine of scraping web-content, followed by a dynamic abort, to adjust the URL, through `restart()`, for which the initial BotAction uses.
