...
These BotAction's scrape the page's `document` and are called ScraperBotAction's. They use a HTML parser to convert the `outerHTML` property of an HTML Element(s) into an interactive object with a similar API to jQuery.

The default HTML parser is [cheerio](https://github.com/cheeriojs/cheerio)'s [load() function](https://github.com/cheeriojs/cheerio#loading).
