...
```
Takes an async function, similar to a `BotAction` (same parameters but called a [ConditionalBotAction](/topic/conditionals#conditional-botaction)) that is expected to return a promise that resolves to a Boolean value. If that promise resolves to `true`, only then, will it run the assembled actions.
