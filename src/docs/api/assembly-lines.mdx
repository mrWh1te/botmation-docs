...
Switch Pipe was built to provide a functional switch statement. Similar to an if statement, but a switch statement tests each *case* condition provided to determine which code blocks to run. Switch Pipe makes this possible by providing the same Pipe object to each BotAction who can then test or operate on it for various cases. 

BotAction's like [pipeCase()()](/api/pipe#pipe-case) or [pipeCases()()](/api/pipe#pipe-cases) test the received Pipe object value against its own provided values, to determine if their assembled BotAction's should run. Both these functions return a `CasesSignal` that can catalyze Switch Pipe into aborting with one less `assembledLines`.

Switch Pipe has unique aborting functionality. By default, an AbortLineSignal with `assembledLines` of `1`, is absorbed by Switch Pipe until it receives a CasesSignal with `conditionPass` is true.

When a CasesSignal's `conditionPass` is `true`, it lowers the required `assembledLines` count to enable breaking the assembled line with an AbortLineSignal with 1 `assembledLines`. This is how a switch, case, break block can be composed functionally.
...
Except for `abort(0)`, which breaks the entire bot, Switch Pipe, in order to absorb `abort(1)` when no cases have matched, it will pre-process received AbortLineSignal's by 1. It does that by reducing the AbortLineSignal's `assembledLines` count by 1, when the Switch Pipe has *yet* to receive a CasesSignal with `conditionPass = true` from an assembled BotAction. Once Switch Pipe receives a CasesSignal with a true `conditionPass`, it stops pre-processes received AbortLineSignal's so `abort(1)` can break the line.

Here's a table to understand it's aborting logic, ran after it has conditionally subtracted `1` from the AbortLineSignal's `assembledLines` count:
