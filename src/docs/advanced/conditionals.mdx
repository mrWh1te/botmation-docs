...
## Conditionals checking Pipe Value

There are other BotAction's like `givenThat()()` for running a line of BotActions if a condition pass. They evaluate the equality of values supplied against the Pipe object value. They are called [pipeCase()()](/api/pipe#pipe-case) && [pipeCases()()](/api/pipe#pipe-cases) which run the assembled BotAction's of their second call against the values provided in their first call.

### Conditional Callback

Instead of handling a ConditionalBotAction in their first call, they expect a Pipe value, which can be almost anything, including a function. When a function is provided, it's called as a callback with the pipe value as its only parameter. These functions are called ConditionalCallback's. Here is the interface:
...
They take a PipeValue and operate on it, in order to return a boolean value, representing the condition of the case past.

These BotAction's support multiple values, and handle their overall if condition differently. `pipeCase()()` will test all of its values as separate expressions to match against the Pipe value. If *one or more* of those expressions evaluates as `true` then it runs its assembled BotAction's. In essence, it combines each value in the conditional expression with `||`.

`pipeCases()()` on the otherhand will test all of its values until one of them evaluates as `false`. All of its values *must* match the Pipe value, in order for the assembled BotAction's to run. In essence, it combines each value in the conditional expression with `&&`.

There are benefits to this approach, as you can supply simple sync functions to create conditional expressions to determine whether or not some functionality needs to run. Both of these will pipe in the same Pipe object that they are testing into their first assembled BotAction's. Therefore, it's possible to nest these to create more elaborate conditions that combine or/and `||`/`&&`. 

You can combine these BotAction's with a [Switch Pipe](/api/assembly-lines#switch-pipe), to create a switch, case(s), and break(s) (with the [Abort](/api/abort#abort) BotAction) flow to specify diversify further the handled use-cases. While [Switch Pipe](/api/assembly-lines#switch-pipe), [Pipe Case](/api/pipe#pipe-case), [Pipe Cases](/api/pipe#pipe-cases), and [Abort](/api/abort#abort) were all created together, to work together in this particular way, they are all BotAction's that can be assembled with or without each other. For an example of all three combined, see [LinkedIn's likeUserPostsFrom()](/sites/linkedin#like-user-posts-from) BotAction.
