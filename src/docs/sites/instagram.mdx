...
```
This BotAction will close the "Turn on Notifications" modal. Please use [givenThat()()](/api/conditionals#given-that) with [isTurnOnNotificationsModalActive](/sites/instagram#isturnonnotificationsmodalactive-conditionalbotaction) to run this BotAction only if that modal is open.
