...
## postIsAuthoredByAPerson ConditionalBotAction
```typescript
...
```
This BotAction is a work in progress since it needs to filter the feeds' posts by type, ie posts representing comments of someone you follow, versus posts representing articles posted by people you follow, etc. Each one has a varying layout, therefore their own unique scraping requirements. 

> See this [Proposal](https://github.com/mrWh1te/Botmation/issues/61) for links with developmental progress and notes.

## toggleMessagingOverlay BotAction
```typescript
