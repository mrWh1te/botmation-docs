...
<img src="/orange_bot.png" alt="Orange Bot" align="right" width="145px" />
...
Imagine a line of people at a coffee shop ☕. As the line forms, the last person who enters the shop joins the end of the line. The first person served is the first person in line. 
...
Let's take the metaphor further, into Alice in Wonderland... come follow the white rabbit 🐇 for a second.

In Botmation, any one of those people in line 🧍, can actually be a whole other line of people 🧑🏾‍🤝‍🧑. Then any one of those people in this deeper line can also be a whole other line of people 🧑🏾‍🤝‍🧑🧑🏾‍🤝‍🧑, and infinitely deep ♾️. It doesn't matter, because a BotAction can be a small specific async operation or a line of other BotAction's.
...
`login()` is a higher-order sync function that customizes the BotAction returned. This returned BotAction is a composition of other BotAction's assembled with `chain()`. This works because the first call of `chain()` returns a BotAction. The second call of chain is the async BotAction that would run the declared BotAction's with the customizing parameters from the the higher-order `login()` call.

This BotAction completes a login flow for an example website. It starts going to the login page, entering the `username` and `password` into their login page's form, then submits the form, and finally waits for the Navigation of the page to complete, before logging `Login Complete` in the NodeJS console.