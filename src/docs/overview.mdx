...
Imagine a door into a coffee shop ☕. As 🧍 🧍 🧍 customers enter, a 🧍🧍🧍 line forms. The first customer in is served first then the next and the next.

BotAction's are declared in lines like that. They run from first to last.

But wait, there's more. Any 🧍 person in 🧍🧍🧍 line, can actually be a whole other 🧍🧍🧍 line of people. And any one of those people in this "child" line, can be a whole other 🧍🧍🧍 line of people, and so on & so forth ♾️🐇. It's all composable, in any order. A BotAction can be a specific async operation or it can be a whole other line of BotAction's.
