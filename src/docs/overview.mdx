...
> If you are new to Functional programming, but are familiar with Object-Oriented, consider a single class that wraps units of functionality. An instance of that class will always have that functionality, even if you don't need it *all*, at times.
> Functional programming looks at a class of functions as separate functions that can be combined, like building blocks, to create more powerful functions. So classes are replaced with compositions of functions. As a metaphor, it's similar to molecules and atoms. Each unique atom is like a unique unit of functionality. Combing them together in a composition creates a system of functionality. You can mix them, match them, replace them, it's a robust structure.
...
Here a Bot is composed with a single horizontal chain of BotAction's. The Bot will first navigate to the URL provided, then wait for navigation to complete. As you can see, we write "await" only only once for the Bot, the composition of it all. Also, each BotAction is being assembled, not called. Because, `chain()`'s first call assembles BotAction's in a line to run, when the second call `chain()()`, the actual BotAction, is awaited. See `chain()()` is like `goTo()()`, they both have a higher order sync function to call first before the BotAction can be resolved.
