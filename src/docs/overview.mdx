...
The higher-order sync function returns the customized BotAction function during runtime. Here [goTo()](/api/navigation/#go-to) uses a higher-order sync function to dynamically set the `url` and `goToOptions?` for the BotAction.
