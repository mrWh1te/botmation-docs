...
BotAction's are async functions that handle various tasks in the web, from specific things like "click this button" to broad flows like "scrape this feed and like my friends' posts". They are *composable*. They can be assembled together to create complex functions that are highly readable. They are like bricks, you can lay them down to build walls, then use the walls to build buildings, then use the buildings to build cities and so forth. Except, each one of these things, even at different levels of composition, are still the same *type* of thing. They can be used & reused anywhere (city on a building, whatever), regardless of how complex their composition is.

Imagine a door into a coffee shop ☕. As customers enter 🧍 🧍 🧍, a line forms 🧍🧍🧍. The first customer served, is the first customer in line, then the next, and the next..

BotAction's can be declared in lines to create bigger BotAction's. They are ran in the order declared, from first to last.

But wait, there's more. In a line 🧍🧍🧍, a person 🧍 can actually be a whole other line of people 🧍🧍🧍. And any one of those people 🧍 in this other line 🧍🧍🧍, can be a whole other line of people 🧍🧍🧍, and so on & so forth ♾️🐇. It's all composable, in any order, regardless of how complex their composition is. A BotAction can be a specific async operation or it can be a whole tree of varying lines of BotAction's.

The possibilities are endless!

Imagine a gold chain of separate individual links. Each link in the chain represents a BotAction. And any link in the chain, can be a whole other chain of links, infinitely deep. Each link is self-reliant, they don't need other links to operate. They are ran one at a time, in-order, until their async functionality resolves. So happenly, a single link, can be a whole tree of chains and links, a composition of complex async functionality. It can all be put together in any way you like, there are no limits.
...
It might look magical, but the TypeScript typing keeps it in check for us.
