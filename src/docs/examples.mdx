...
## Screenshots Bot
This [screenshots example](https://github.com/mrWh1te/Botmation/tree/master/src/examples/screenshots.ts) demonstrates the power of Puppeteer in the ease of generating PNG screenshots, and ease of Botmation, in composing a bot that saves screenshots of many websites. Let's see it in action:
...
> It uses an Utility BotAction called [forAll()()](/api/utilities#for-all)

## Instagram Bot

The [Instagram example](https://github.com/mrWh1te/Botmation/tree/master/src/examples/instagram.ts) is built with BotAction's made for Instagram's web app. They are included in Botmation v2.x. 
...
[isGuest](/sites/instagram#isguest-conditionalbotaction) and [isLoggedIn](/sites/instagram#isloggedin-conditionalbotaction) are a special kind of BotAction called ConditionalBotAction. These two, and [login()](/sites/instagram#login-botaction) are part of the `Botmation` npm package that can be imported for you to use and are specific for Instagram's web app.

> It's currently planned to support Instagram's auth BotAction's inside the Botmation npm package until major v3.0.0. Soon, to be released as a separate new npm package ie `botmation-instagram` will provide the same BotAction's with the same license. There will be separate npm packages for each collection of BotAction's focused on each website like Twitter, Instagram, LinkedIn, Reddit, etc.
...
## Object-Oriented Bot
Classes are great for encapsulating functionality, but make it harder to reuse composed functionality. Regardless, it's possible to use Botmation to compose instance functions that operate on the bot's `page`. Here's an example:
